...
interface Run { id: string; due_at_iso: string; started_at_iso?: string; status: string; attempt: number; error?: string; output_path?: string; attempts: Attempt[] }
...
export function ScheduleRuns({ id }: { id: string }) {
  ...
  const format = (at: string) => at.replace('T', ' ').slice(0, 19);
  return <div className="space-y-2 text-xs">
    ...
    {loading ? <p>正在读取执行记录…</p> : error ? <p role="alert" className="text-danger">{error}</p> : !runs.length ? <p className="text-text-muted">还没有到期执行记录。</p> : runs.map(run => <div key={run.id} className="rounded border border-border p-3 space-y-1">
      <div className="flex flex-wrap gap-3"><span>计划 {format(run.due_at_iso)}</span><strong>{LABELS[run.status] || run.status}</strong><span className="text-text-muted">已尝试 {run.attempt} 次</span></div>
